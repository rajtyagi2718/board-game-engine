...
class DisjointSet():
    ...
    def component(self, i):
        ...
        p = self._find(i)
        return tuple(j for j in range(len(self)) 
                     if self._find(j) == p)
    ...
    def undo(self):
        ...
        elif isinstance(connection, Atoms):
            for i,w in zip(connection.component, connection.weight):
                self._weight[i] = w
            for i,p in zip(connection.component, connection.parent):
                self._parent[i] = p
...
LibertiesAtoms = namedtuple('Atoms', 'component weight parent liberties i')

class LibertiesDisjointSet(DisjointSet):
    """Extension of Disjoint Set with set of liberties per component."""

    def __init__(self, size, adjs):
        super().__init__(size)
        self._liberties = np.array([set(adj) for adj in adjs], dtype=object)
        self._adjs = adjs
    ...
    def remove(self, i, lib):
        """Remove liberty lib from component i, all parents."""
        while i != self._parent[i]:
            try:
                self._liberties[i].remove(lib)
            except KeyError:
                pass
    ...
    def atomize(self, i):
        ...
        weight = tuple(self._weight[i] for i in component)
        parent = tuple(self._parent[i] for i in component)
        liberties = tuple(self._liberties[i] for i in component)
        self._connections.append(
            LibertiesAtoms(component, weight, parent, liberties, i))
        # print('*'*10 + 'Atomize %d!' % i + '*'*10) 
        for i in component:
            self._weight[i] = 1
            self._parent[i] = i
        # add liberties after component disconnect
        for i in component:
            for adj in self._adjs[i]:
                # print('*'*10 + 'Add liberty %d to %d!' % (i, adj) + '*'*10) 
                self._liberties[self._find(adj)].add(i)

    def undo(self, i, j=None):
        """Undo last call to connect. Use last connection to restore structs"""
        connection = self._connections.pop() 
        if isinstance(connection, LibertiesConnection):
            ...
            if connection.i == i:
                ...
                for i,p in zip(connection.component, connection.parent):
                    self._parent[i] = p
                for i,b in zip(connection.component, connection.liberties):
                    self._parent[i] = b
            else:
                raise IndexError('cannot undo. last atomize was %r' 
                                 % connection)
        # if connection:
        #     self._parent[connection.slave] = connection.slave
        #     self._weight[connection.master] -= self._weight[connection.slave]
    ...
    def __repr__(self):
        ...
        result = zip(range(81), self._weight, self._parent,
                     (sorted(libs) for libs in self._liberties))
        return '\n'.join(map(str, result))

    def __str__(self):
        """Return grid of components.""" 
        result = [ALPHABET[self._find(i)] for i in range(len(self))]
        result = [' '.join(result[i:i+9]) for i in range(0, 81, 9)]
        return '\n'.join(result) 
